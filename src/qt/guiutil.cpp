...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a IKT address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no IKT: URI
...
{
    // Convert innoket:// to innoket:
    //
    //    Cannot handle this later, because innoket:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open innoket.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "IKT.lnk";
}
...
{
    return GetAutostartDir() / "innoket.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a innoket.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the innoket app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add innoket app to startup item list
