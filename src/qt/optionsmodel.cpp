...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeInnoketAmount"))
        settings.setValue("nAnonymizeInnoketAmount", 1000);

    nAnonymizeInnoketAmount = settings.value("nAnonymizeInnoketAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeInnoketAmount"))
        SoftSetArg("-anonymizeinnoketamount", settings.value("nAnonymizeInnoketAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in innoket.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeInnoketAmount:
            return QVariant(nAnonymizeInnoketAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeInnoketAmount:
            nAnonymizeInnoketAmount = value.toInt();
            settings.setValue("nAnonymizeInnoketAmount", nAnonymizeInnoketAmount);
            emit anonymizeInnoketAmountChanged(nAnonymizeInnoketAmount);
            break;
        case CoinControlFeatures:
